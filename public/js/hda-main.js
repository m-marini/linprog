 ...
 * 
 */
window.MainApp = function(window) {
	var $ = window.$;
	...
	function renderProduction(data) {
		clearTable('prodConf');
		var rows = _(hdaApi.productOrder).filter(productExists).map(toUIModel)
				.value();
	...
	function renderMarket(data) {
		...
		function image(key) {
			return '<img src="' + imagePath(key) + '" alt="Image of '
					+ productName(key) + '" class="img-rounded hda-image">';
		}
		...
		function toHtml(key) {
			return [
					label(key),
					image(key),
					builder.createFieldHtml(key + '-val', data.values[key],
							'Price of ' + productName(key)) ];
		}
	...
	function renderConf(data) {
		...
		function image(key) {
			return '<img src="' + imagePath(key) + '" alt="Image of '
					+ supplierName(key) + '" class="img-rounded hda-image">';
		}
		...
		function toHtml(key) {
			return [
					label(key),
					image(key),
					builder.createFieldHtml(key + '-qty', data.suppliers[key],
							'Number of ' + supplierName(key)) ];
		}
	}
	...
	function renderTable(target, content) {
		var html = _(content).map(toRows).map(prePostFix('<tr>', '</tr>'))
				.reduce(concat);
		target.append(html);

		function toRows(cols) {
			return _(cols).map(prePostFix('<td>', '</td>')).reduce(concat);
		}
	}
